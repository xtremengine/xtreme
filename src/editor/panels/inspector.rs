...
impl InspectorPanel {
    ...
    pub fn show(
        ...
        if let Some(ref mut camera) = obj.camera {
            ...
            let header = egui::CollapsingHeader::new("Camera")
                .default_open(self.camera_expanded);
            ...
            header.show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("Priority:");
                    if ui.add(egui::DragValue::new(&mut camera.priority).speed(1)).changed() {
                        changed = true;
                    }
                });
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.selectable_label(!is_perspective, "Orthographic").clicked() && is_perspective {
                ...
                match &mut camera.projection {
                    CameraProjection::Perspective { fov, near, far } => {
                        ui.horizontal(|ui| {
                            ui.label("FOV:");
                            if ui.add(egui::DragValue::new(fov)
                                .range(10.0..=170.0)
                                .speed(1.0)
                                .suffix("°")
                        ...
                        ui.horizontal(|ui| {
                            ui.label("Near:");
                            if ui.add(egui::DragValue::new(near)
                                .range(0.001..=100.0)
                                .speed(0.01)
                        ...
                        ui.horizontal(|ui| {
                            ui.label("Far:");
                            if ui.add(egui::DragValue::new(far)
                                .range(1.0..=100000.0)
                                .speed(10.0)
                                .fixed_decimals(1)).changed() {
                                changed = true;
                    ...
                    CameraProjection::Orthographic { size, near, far } => {
                        ui.horizontal(|ui| {
                            ui.label("Size:");
                            if ui.add(egui::DragValue::new(size)
                                .range(0.1..=1000.0)
                                .speed(0.1)
                                .fixed_decimals(1)).changed() {
                                changed = true;
                ...
                ui.horizontal(|ui| {
                    ...
                    let mut color = [camera.clear_color[0], camera.clear_color[1], camera.clear_color[2]];
                    if ui.color_edit_button_rgb(&mut color).changed() {
